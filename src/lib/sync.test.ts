...
// ---------------------------------------------------------------------------
// Conflict resolution: isConflicting / makeConflictCopy
// ---------------------------------------------------------------------------

describe('isConflicting', () => {
  it('flags a server version newer than a queued local edit', () => {
    const row = { updated_at: new Date(3000).toISOString() } as never
    expect(isConflicting(row, note())).toBe(true)
  })

  it('does not conflict when the local edit is newer', () => {
    const local = note({ updatedAt: 3000 })
    const row = { updated_at: new Date(2000).toISOString() } as never
    expect(isConflicting(row, local)).toBe(false)
  })

  it('does not conflict on identical timestamps', () => {
    const row = { updated_at: new Date(2000).toISOString() } as never
    expect(isConflicting(row, note())).toBe(false)
  })
})

describe('makeConflictCopy', () => {
  it('creates a new id and marks the title as a conflicted copy', () => {
    const copy = makeConflictCopy(note()) as NoteItem
    expect(copy.id).not.toBe('note-1')
    expect(copy.title).toContain('(conflicted copy)')
  })

  it('uses Untitled for blank titles and preserves the body', () => {
    const copy = makeConflictCopy(note({ title: '' })) as NoteItem
    expect(copy.title).toContain('Untitled')
    expect(copy.text).toBe('World')
    expect(copy.deleted).toBe(false)
  })

  it('passes non-note items through unchanged', () => {
    const tag = { ...note(), type: 'tag' } as unknown as Item
    expect(makeConflictCopy(tag).id).toBe('note-1')
  })
})
